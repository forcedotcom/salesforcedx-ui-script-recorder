// Ported from @fpsx/ui-recorder-utils/src/injected/selectors/ARIASelector.ts
// Copyright 2023 The Chromium Authors. All rights reserved.

/**
 * ARIA selector computer that traverses shadow DOM.
 * Uses accessibility bindings to query the a11y tree.
 */
class ARIASelectorComputer {
   ...
   * Traverses through shadow roots.
   */
  #queryA11yTree = (parent, name, role, maxResults = 0) => {
    const result = []
    if (!name && !role) throw new Error('Both role and name are empty')
 ...
 * Traverses shadow DOM and uses accessibility bindings.
 */
